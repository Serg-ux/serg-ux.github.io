...
tags: [DFIR, Malware Analysis, Sherlock, windows, Very Easy]
---
...
_Sysmon Operational log exported from the victim host, ready to be loaded in Event Viewer._
...
_Event Viewer's "Filter Current Log" action, used to narrow the dataset to a specific Event ID._

![Filtering by Event ID 11](/assets/images/unit42-backdoored-ultravnc-dropper/03-filter-event-id-11.png)
_Filter dialog configured to return only FileCreate events._
...
_Six ProcessCreate events recorded around the time of infection._
...
_Full ProcessCreate event detail, including PE metadata, hashes, and parent process._
...
_VirusTotal classifies the file under the family labels `winvnc`, `based`, and `ultravnc`._
...
_Community graph titled "Dropbox Malware", indicating Dropbox was used to host and distribute the payload._
...
_FileCreateTime event for the dropped PDF decoy, showing both the new and the original timestamp._
...
_FileCreateTime event exposing the full path of `once.cmd`._
...
_DNS query issued by the malicious process for `www.example.com`._
...
_TCP connection from the malicious process to `93.184.216.34` on port 80._
...
_ProcessTerminate event for `Preventivo24.02.14.exe.exe`._
